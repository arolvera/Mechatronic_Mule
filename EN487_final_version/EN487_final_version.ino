...
// Modified by: Anthony Olvera for Colorado State University-Pueblo
//
//==========================================================================
...
#include <SPI.h>  
#include <Pixy.h>
#include <PixySPI_SS.h>//modified
#include <stdint.h>
//#include <unistd.h>
//#include <pixydefs.h>
#include <SabertoothSimplified.h> // modified
SabertoothSimplified ST; //modified
//#include <ZumoMotors.h> modified
...
void setup()
{
  pinMode(lowVPin, OUTPUT);          // Declare low voltage indicator pin as an output 
  SabertoothTXPinSerial.begin(9600); // modified for sabertooth motor driver
  Serial.begin(9600);
...
void loop()
{ 
  int sensorValue = analogRead(A0); // Read signal from analog input
  float voltage = 11.3636363636*(sensorValue*(5.0/1023.0)); // Convert Signal to voltage
  if (voltage < 10) // Iluminate low voltage LED if voltage is below desired value
  ...
  else if (millis() - lastBlockTime > 100)
  {
    ...
    ST.motor(1, 0); //modified for sabertooth
    ST.motor(2, 0); //modified for sabertooth
    ScanForBlocks();
  }
}
...
//---------------------------------------
// Follow blocks via the Sabertooth 2X25 driver
//
...
void FollowBlock(int trackedBlock)
{
  ...
  int forwardSpeed = constrain(400 - (size/256), -100, 400); //modified

  // Steering differential is proportional to the error times the forward speed
  int32_t differential = (followError + (followError * forwardSpeed))>>8;

  // Adjust the left and right speeds by the steering differential.
  int leftSpeed = constrain(forwardSpeed + differential, -400, 400); //modified
  int rightSpeed = constrain(forwardSpeed - differential, -400, 400); //modified 

  // And set the motor speeds
  ST.motor(1, leftSpeed);//modified
  ST.motor(2, rightSpeed);//modified
...
void ScanForBlocks()
{
  if (millis() - lastMove > 20)
  {
    ...
    if ((panLoop.m_pos >= RCS_MAX_POS)||(panLoop.m_pos <= RCS_MIN_POS))
    {
      ...
      if (scanIncrement < 0)
      {
        ST.motor(1,0); //modified
        ST.motor(2,0);  //modified
